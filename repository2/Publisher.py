    ...
    while True:
        ...
        for p in partite:  #scorre le partite
            if not p["iniziata"] and adesso >= p["ora_inizio"]:  #se NON è inziata e è arrivato il momento di inizio (di quella partita)
                ...
                in_corso.insert_one(p)
                programmati.delete_one({"id":p["id"]})  #elimina quella con l'id giusto
                p=Partita(p)
                p.start()


        if partite[4]["iniziata"]==True: #se anche l'ultima, quindi quella che inizia per ultima, è inziata
    ...
    async def start(self):
        ...
        while tempo < durata:
            ...
            if random.randint(0, 1) == 1:
                ...
                if evento == 1:
                    self.punti += 1
                    in_corso.update_one({"id": self.id}, {"$inc": {"punti":1}})
                elif evento == 2:
                    self.ammonizioni += 1
                    in_corso.update_one({"id": self.id}, {"$inc": {"ammonizioni": 1}})
                elif evento == 3:
                    self.interruzioni += 1
                    in_corso.update_one({"id": self.id}, {"$inc": {"interruzioni": 1}})
            #
...
if __name__ =="__main__":
    partite=programmati.find() #lista dei programmati
    avvia_partite(partite)  #inizia il controllo per avviarle
